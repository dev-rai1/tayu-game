...
describe('Aug. 9 comprehensive playtest regressions', () => {
  ...
  it('keeps the jar HUD, Skip Talk, and Admin controls in separate screen anchors', () => {
    ...
    expect(admin).toContain('bottom-[calc(4.25rem+env(safe-area-inset-bottom,0px))]')
  })
  ...
  it('keeps privacy choices compact and completely off immersive gameplay screens', () => {
    ...
    expect(source).toContain('if (!canAllowAnalytics || choice || isImmersive')
    expect(source).toContain('Optional analytics are off unless you choose to allow them.')
