...
export class Boundary extends Component {
  ...
  componentDidCatch(err) { logTayuError('boundary:' + (this.props.name || 'app'), err?.message || err) }
  render() {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-navy p-6 text-center text-white">
        ...
        <button
          ...
          onClick={() => { this.setState({ oops: false }); if (this.props.hard) window.location.reload() }}
        >
          Try again
        </button>
      </div>
