...
function makeRecap({ title, memory, steps }) {
  return {
    ...
    memory,
    steps,
    // Keep `items` for any legacy/read-only surfaces that still consume recaps.
...
const MODULE_RECAPS = {
  ...
  5: makeRecap({
    ...
    steps: [
      { activity: 'Read the W-2', lesson: 'A W-2 reports wages and tax already withheld. Those records are starting numbers for the practice return.' },
      { activity: 'Find taxable income', lesson: 'The practice deduction reduces the wages used in the tax calculation, so taxable income can be lower than total wages.' },
      { activity: 'Use the tax brackets', lesson: 'Different parts of taxable income can use different rates. You calculated each part and added the tax together.' },
      { activity: 'Apply a tax credit', lesson: 'A tax credit reduces calculated tax directly after the bracket math.' },
      { activity: 'Compare withholding and final tax', lesson: 'If withholding is greater than final tax, the difference can be a refund. If final tax is greater, the difference can be an amount due.' },
      { activity: 'Review and file', lesson: 'Before filing, check that the income, taxable income, tax, credits, withholding, and refund or amount due agree with the return.' },
    ],
  }),
  6: makeRecap({
    title: 'Money Garden',
    ...
    steps: [
      { activity: 'Buy company shares', lesson: 'A stock is a small ownership interest in a company, and a share is one unit of that ownership. Its return can be a gain or a loss, so investing always involves uncertainty.' },
      { activity: 'Spread your investments', lesson: 'Diversification spreads money across investments so one company has less control over the result. It can reduce concentration risk, but it cannot eliminate every loss.' },
      { activity: 'Research the businesses', lesson: 'Credible business evidence matters more than a low price, a recent jump, or hype. Price alone does not tell you whether a company is healthy.' },
      { activity: 'Keep some money ready', lesson: 'Your time horizon is how long until you need the money. Keeping near-term needs in ready cash can keep a surprise from forcing a sale at a bad time.' },
      { activity: 'Watch several weeks', lesson: 'One dramatic week can reverse. Patient investing means comparing new evidence over time instead of chasing a short-term move.' },
      { activity: 'Rebalance the garden', lesson: 'Rebalancing means adjusting holdings after prices change so the portfolio moves back toward its intended mix and risk level.' },
    ],
  }),
}
...
export function WorldModuleLearningRecap() {
  ...
  const triggerModule = useMemo(() => {
    ...
    // Module 5 owns its filing result/lesson recap inside PaycheckPlanetWorld,
    // so do not open a second full-screen recap on the same completion signal.
