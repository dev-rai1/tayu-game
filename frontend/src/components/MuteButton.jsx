  ...
  return (
    <div className="notranslate relative mt-2" translate="no">
      ...
      <button
        ...
        className="pointer-events-auto flex min-h-10 w-full items-center justify-center gap-2 rounded-2xl bg-white/95 px-3 py-2 text-sm font-extrabold text-navy shadow-lg transition active:scale-95"
      >
        <span aria-hidden="true">🌐</span>
        <span>Language</span>
        <span className="rounded-lg bg-navy/10 px-2 py-0.5 text-xs">{active.short}</span>
      </button>
        ...
        <div className="pointer-events-auto absolute left-0 top-full z-[900] mt-2 grid max-h-[65vh] min-w-[270px] grid-cols-2 gap-2 overflow-y-auto rounded-2xl border border-white/30 bg-navy/95 p-2 shadow-2xl backdrop-blur-md sm:grid-cols-3">
          {LANGUAGES.map((item) => (
            <button
              ...
              className={`rounded-xl px-3 py-2 text-left text-sm font-extrabold transition active:scale-95 ${
                language === item.code
                  ? 'bg-mint text-navy'
  ...
  return (
    <div className={`pointer-events-auto inline-flex flex-col ${className}`}>
      <button
        ...
        className="flex h-11 items-center justify-center gap-2 rounded-2xl bg-navy/80 px-3 text-white shadow-lg transition active:scale-95"
      >
