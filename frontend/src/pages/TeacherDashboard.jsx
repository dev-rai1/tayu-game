    ...
    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Link to="/modules?teacherPreview=1" className="btn-primary">Try my session</Link>
          <button type="button" onClick={loadStudents} disabled={loadingStudents} className="rounded-xl bg-white/10 px-4 py-3 font-extrabold disabled:opacity-50">{loadingStudents ? 'Refreshing…' : 'Refresh analytics'}</button>
          <button type="button" onClick={exportCsv} disabled={!students.length} className="rounded-xl bg-white/10 px-4 py-3 font-extrabold disabled:opacity-40">Export detailed CSV</button>
          <Link to="/" className="rounded-xl bg-white/10 px-4 py-3 font-extrabold">Home</Link>
        </div>
      </header>
      ...
      <section className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-8">
        ...
        <Stat label="Pre surveys" value={stats.pre} />
        <Stat label="Post surveys" value={stats.post} />
        <Stat label="Certificates" value={stats.certificates} />
        <Stat label="Avg verified change" value={stats.avgChange === null ? '—' : `${stats.avgChange >= 0 ? '+' : ''}${stats.avgChange.toFixed(2)}`} />
      </section>

      <section className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5">
        <h2 className="font-display text-xl font-extrabold">Class time spent by module</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {MODULES.map((moduleName) => <div key={moduleName} className="rounded-xl bg-black/20 p-4"><div className="text-sm font-extrabold text-teal">{MODULE_LABEL[moduleName]}</div><div className="mt-2 text-2xl font-extrabold">{duration(stats.moduleTime[moduleName] || 0)}</div></div>)}
        </div>
      </section>
      ...
      <section className="mt-6 overflow-x-auto rounded-2xl border border-white/10 bg-white/5 p-5">
        ...
        {loadingStudents && !students.length ? <p className="mt-4 text-white/55">Loading detailed analytics…</p> : (
          <table className="mt-4 w-full min-w-[1250px] text-left text-sm">
            <thead><tr className="text-xs uppercase text-white/45"><th className="py-2 pr-3">Student</th><th className="pr-3">Grade</th><th className="pr-3">Joined</th><th className="pr-3">Logins</th><th className="pr-3">Last login</th><th className="pr-3">Sessions</th><th className="pr-3">Active time</th><th className="pr-3">Modules</th><th className="pr-3">Certificate</th><th>Verified pre → post</th></tr></thead>
            <tbody>{students.map((student) => {
              const assessment = student.progress?.profile?.assessment || {}
              const pre = verifiedScore(assessment.pre)
              const post = verifiedScore(assessment.post)
              return <tr key={student.uid} onClick={() => setSelectedUid(selectedUid === student.uid ? '' : student.uid)} className="cursor-pointer border-t border-white/10 hover:bg-white/5"><td className="py-3 pr-3 font-extrabold">{student.email || 'No email'}</td><td className="pr-3">{student.gradeLevels || '—'}</td><td className="pr-3">{timestamp(student.joinedClassAt || student.createdAt)}</td><td className="pr-3 font-extrabold">{student.loginCount}</td><td className="pr-3">{timestamp(student.lastLoginAt, 'Not yet')}</td><td className="pr-3">{student.sessions.length}</td><td className="pr-3">{duration(totalSessionSeconds(student.sessions))}</td><td className="pr-3">{student.completed}/5</td><td className="pr-3">{student.certificateEarned ? 'Earned' : 'Not yet'}</td><td className="font-extrabold">{pre === null ? 'Not taken' : `${pre}/${KNOWLEDGE_QUESTIONS.length} → ${post === null ? 'pending' : `${post}/${KNOWLEDGE_QUESTIONS.length} (${post - pre >= 0 ? '+' : ''}${post - pre})`}`}</td></tr>
            })}</tbody>
          </table>
        )}
      ...
      <section className="mt-6 overflow-x-auto rounded-2xl border border-white/10 bg-white/5 p-5">
        <h2 className="font-display text-xl font-extrabold">Class login and logout timestamps</h2>
        {classActivity.length === 0 ? <p className="mt-3 text-sm text-white/55">No login history has been recorded yet.</p> : <table className="mt-4 w-full min-w-[760px] text-left text-sm"><thead><tr className="text-xs uppercase text-white/45"><th className="py-2 pr-3">Event</th><th className="pr-3">Student</th><th className="pr-3">Timestamp</th><th className="pr-3">Device</th><th>Page</th></tr></thead><tbody>{classActivity.slice(0, 250).map((event) => <tr key={event.id} className="border-t border-white/10"><td className="py-2 pr-3 font-extrabold text-teal">{EVENT_LABEL[event.type] || event.type}</td><td className="pr-3">{event.email || '—'}</td><td className="pr-3">{timestamp(event.occurredAt)}</td><td className="pr-3">{event.device || 'Unknown'}</td><td>{event.path || '—'}</td></tr>)}</tbody></table>}
      </section>
    </main>
  )
}
  ...
  return (
    <section className="mt-6 rounded-2xl border-2 border-teal/40 bg-white/5 p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat label="Successful logins" value={student.loginCount} />
        <Stat label="Recorded sessions" value={student.sessions.length} />
        <Stat label="Total active time" value={duration(totalSessionSeconds(student.sessions))} />
        <Stat label="Last activity" value={timestamp(student.lastActiveAt || student.sessions[0]?.lastSeenAt)} />
      </div>

      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl bg-black/20 p-3"><div className="text-xs font-extrabold uppercase text-white/50">Last login</div><div className="mt-1 font-bold">{timestamp(student.lastLoginAt, 'Not yet')}</div></div>
        <div className="rounded-xl bg-black/20 p-3"><div className="text-xs font-extrabold uppercase text-white/50">Last logout</div><div className="mt-1 font-bold">{timestamp(student.lastLogoutAt, 'Not recorded')}</div></div>
        <div className="rounded-xl bg-black/20 p-3"><div className="text-xs font-extrabold uppercase text-white/50">Current module</div><div className="mt-1 font-bold">Module {student.currentModule}</div></div>
        <div className="rounded-xl bg-black/20 p-3"><div className="text-xs font-extrabold uppercase text-white/50">Progress</div><div className="mt-1 font-bold">{student.completed}/5 modules · {student.certificateEarned ? 'Certificate earned' : 'Certificate not yet earned'}</div></div>
      </div>
