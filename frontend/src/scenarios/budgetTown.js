...
export const DAY_INTRO = (amount) => `You earned $${amount}. Cover today's needs, decide whether to spend on a want, and keep something ready for a surprise.`
export const STOPS = {
...
export function sliceLine(id, pct) { if (id === 'pocket') return `${pct}% is ready for surprises, but it does not grow.`; if (id === 'bank') return `${pct}% grows slowly with less risk and stays easier to reach.`; if (id === 'bond') return `${pct}% is in bonds. The borrower promises interest and repayment. It is not as safe as the bank, but it usually wiggles less than stocks.`; return `${pct}% is in the Money Garden for higher growth potential and higher stock-market risk.` }
export function splitNudge(split, total) { if (split.pocket >= total) return 'Everything is ready now, but nothing is growing. Compare Bank, Bonds, and Money Garden.'; if (split.garden >= total) return 'Everything is taking stock-market risk. What would pay for a surprise today, and where could steadier choices fit?'; if (split.bank >= total) return 'Everything is in the bank. Compare access now with bond interest and stock growth.'; if (split.bond >= total) return 'Everything is being lent through bonds. Keep some ready cash and compare bank safety with stock growth.'; if (split.pocket === 0) return 'No money is ready for a surprise. Revise the plan before testing it.'; if (split.pocket < Math.min(2, total)) return 'The ready-cash cushion may not cover the surprise. Check the amount again.'; if (split.bond === 0) return 'Bonds are the middle ground — steadier than stocks and designed to pay interest. Worth a small share in most plans.'; if (split.garden > Math.round(total * 0.7)) return 'Most of the plan depends on the stock market. Is that risk balanced?'; return 'This plan includes ready cash, bank savings, bond lending, and higher-risk stock growth.' }
...
export const HANDOFF = (bank, bondOrGarden, gardenMaybe) => { const hasBond = gardenMaybe !== undefined, bond = hasBond ? bondOrGarden : 'your Bond Street reserve', garden = hasBond ? gardenMaybe : bondOrGarden; return `Your revised plan kept emergency cash ready, placed $${bank} in the Bank, ${hasBond ? `reserved $${bond} for Bonds` : `saved ${bond} for Bonds`}, and placed $${garden} in the Money Garden.` }
export function defaultSplit(total) { const pocket = Math.max(2, Math.round(total * 0.2)), bank = Math.round(total * 0.3), bond = Math.round(total * 0.2); return { pocket, bank, bond, garden: Math.max(0, total - pocket - bank - bond) } }
