...
export const OPENING = [
  'A stock is a small ownership piece of a company. One share is one unit of that ownership.',
  'Your return is how much an investment gains or loses in value. Higher possible returns usually come with more risk, which means more uncertainty and a bigger chance of losing value.',
  'Diversification means spreading money across different investments so one company cannot control your whole result. It lowers concentration risk, but it cannot remove all investing risk.',
  'Your time horizon matters too. Money you may need soon should stay ready in Pocket or Bank instead of depending on a market price being high on the exact day you need it.',
  'In this garden you will research businesses, compare evidence with hype, stay patient through normal price moves, and rebalance when one holding becomes too large.',
]
...
export const WEEKS = [
  { n: 1, lesson: 'A stock is ownership: research the business before buying', intro: 'A share makes you a tiny owner of a company. Compare all three businesses first, then choose a portfolio based on evidence instead of guessing which price will jump next.', praise: 'You connected the stock choice to the company behind it and made an evidence-based portfolio decision.', coach: 'The seeds are planted. Keep comparing what the businesses do with what their prices do.', nudge: 'Next: learn why one company should not control the whole portfolio.', special: 'seeds', learn: 'stocks', ctx: () => ({}), fx: () => ({}), moves: (context, garden) => ({ early: {}, late: Object.fromEntries(Object.keys(garden.companies).filter((id) => garden.companies[id].owned > 0).map((id) => [id, +1])) }), judge: (garden) => Object.keys(garden.companies).some((id) => garden.companies[id].owned > 0) },
