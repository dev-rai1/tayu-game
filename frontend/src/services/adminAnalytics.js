...
export async function adminAnalyticsData() {
  ...
  const [profilesSnapshot, progressSnapshot, activitySnapshot, usageSnapshot] = await Promise.all([
  ...
  const sessions = usageSnapshot.docs
  ...
  const accounts = profilesSnapshot.docs.map((item) => {
    ...
    return {
      ...
      sessions: sessionsByUid[item.id] || [],
    }
  })
  ...
  return { accounts, activity, sessions }
}
