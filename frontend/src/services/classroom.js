...
export const DEFAULT_MODULES = [1, 2, 3, 4, 5, 6, 7]
...
export async function saveTeacherClassSettings(settings) {
  const user = currentUser()
  if (!user?.id || user.role !== 'teacher') throw new Error('Teacher account required.')
  const enabledModules = [...new Set((settings.enabledModules || []).map(Number).filter((n) => n >= 1 && n <= 7))].sort()
...
export async function loadTeacherStudents() {
  ...
  const rows = await Promise.all(profiles.docs.map(async (profileDoc) => {
    ...
    const assignedTotal = progress?.profile?.activeLearningPath?.modules?.length || DEFAULT_MODULES.length
    return {
      uid: profileDoc.id,
      email: profile.email || '',
      ...
      amountDone: `${Math.min(badges.length, assignedTotal)}/${assignedTotal}`,
