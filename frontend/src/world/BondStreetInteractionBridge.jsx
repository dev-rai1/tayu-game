...
function placeAtEntrance() {
  try {
    ...
    playerPos.x = BOND_GUIDE_POINT[0]
    playerPos.z = BOND_GUIDE_POINT[1] + 3.2
    joystick.x = 0
    joystick.y = 0
    moveTarget.x = null
    moveTarget.z = null
...
export function BondStreetInteractionBridge() {
  useEffect(() => {
    placeAtEntrance()
    const refresh = () => {
      const near = distanceToGuide() <= INTERACT_RADIUS
      useBondStreet.getState().setNearby(near)
    }
    const interact = () => {
      const store = useBondStreet.getState()
      if (store.panelOpen) return
      if (distanceToGuide() <= INTERACT_RADIUS || store.phase === 'intro') store.open()
    }
    const onKeyDown = (event) => {
      if (event.code !== 'KeyE' || isTypingTarget(event.target)) return
      if (distanceToGuide() > INTERACT_RADIUS && useBondStreet.getState().phase !== 'intro') return
    ...
    return () => {
      ...
      useBondStreet.getState().setNearby(false)
    }
  }, [])
  return null
}
...
export function BondActionPrompt() {
  const nearby = useBondStreet((s) => s.nearby)
  const panelOpen = useBondStreet((s) => s.panelOpen)
  if (!nearby || panelOpen) return null
  return (
    <div className="pointer-events-none fixed bottom-[calc(6.25rem+env(safe-area-inset-bottom,0px))] left-1/2 z-[640] -translate-x-1/2 rounded-2xl border-2 border-white/40 bg-navy/95 px-4 py-2 text-center text-sm font-extrabold text-white shadow-2xl">
      Press E / tap interact to talk to Beau at Bond Street
    </div>
  )
}
