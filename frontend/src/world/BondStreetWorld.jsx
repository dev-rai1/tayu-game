...
function ReactiveNpc({ position, avatar, reacting = false, name }) {
  ...
  useFrame((_, delta) => {
    ...
    const bounce = reacting ? Math.abs(Math.sin(clock.current * 7)) * 0.28 : Math.sin(clock.current * 2.1) * 0.025
  ...
  return (
    <group ref={root} position={position} aria-label={name}>
      <CharacterMesh ref={body} avatar={avatar} />
    </group>
  )
}
      ...
      {Array.from({ length: 10 }, (_, i) => {
        ...
        return (
          <mesh key={i} position={[Math.cos(angle) * 2.4, 1.1 + (i % 2) * 0.55, Math.sin(angle) * 2.4]} rotation={[Math.PI / 2, 0, angle]}>
            ...
            <meshStandardMaterial color="#ffd700" emissive="#ffd700" emissiveIntensity={active ? 0.75 : 0.12} metalness={0.65} roughness={0.28} />
          </mesh>
        )
      })}
    </group>
  ...
  return (
    <group>
      ...
      <Billboard position={[-7.82, 6.55, 0]}>
        <mesh><planeGeometry args={[8.2, 2.15]} /><meshBasicMaterial map={labelTexture('MODULE 6 · BOND STREET', { bg: '#071748', color: '#ffffff', accent: '#9ed36f' })} transparent toneMapped={false} depthTest={false} /></mesh>
      </Billboard>
...
export function BondStreetWorld() {
  ...
  useEffect(() => {
    if (selectedBondArrival) placeAtBondStreetEntrance()
  }, [selectedBondArrival])
