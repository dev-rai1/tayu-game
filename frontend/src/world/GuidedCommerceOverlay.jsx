...
const MOBILE_SHEET = 'pointer-events-auto fixed left-1/2 bottom-[calc(1rem+env(safe-area-inset-bottom,0px))] z-[475] w-[min(94vw,40rem)] max-h-[min(74vh,42rem)] -translate-x-1/2 overflow-y-auto overscroll-contain rounded-3xl shadow-2xl'
const COMPACT_BUTTON = 'pointer-events-auto fixed left-1/2 bottom-[calc(1rem+env(safe-area-inset-bottom,0px))] z-[475] min-h-[48px] w-[min(92vw,24rem)] -translate-x-1/2 rounded-2xl border-2 border-white/20 bg-navy/95 px-4 text-center text-sm font-extrabold text-white shadow-xl active:scale-95'
...
function MarketClickShop() {
  ...
  return (
    <aside className={`${MOBILE_SHEET} border-2 border-teal bg-navy/95 p-4 text-white`}>
...
// Instructional Lemonade coaching now lives in the shared right-side guidance
// rail. This component only owns the actual market interaction surface.
