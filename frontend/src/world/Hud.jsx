          ...
          {BUNDLES.map((b) => {
            ...
            return (
              <button key={b.id} disabled={!ok} onClick={() => choose(b.id)}
                ...
                <div className="mt-1 text-[10px] font-bold leading-tight text-teal">cups + lemons + sugar + water + table</div>
              </button>
            )
          })}
        </div>
  ...
  return (
    <div className="pointer-events-auto absolute inset-0 z-[290] flex items-end justify-center bg-navy/30 p-4 sm:items-center">
      <div role="dialog" aria-modal="true" aria-label="Game help" className="glass--navy pop-in max-h-[92vh] w-full max-w-md overflow-y-auto p-5 text-center">
        ...
        {tab === 'resources' && (
          <div id="help-panel-resources" role="tabpanel">
...
function ModuleTransition() {
  ...
  return (
    <div className="pointer-events-auto absolute inset-0 z-[360] flex items-center justify-center bg-navy/75 p-4 backdrop-blur-sm">
      <div role="dialog" aria-modal="true" aria-labelledby="module-transition-title" className="pop-in w-full max-w-md rounded-3xl bg-white p-7 text-center shadow-2xl">
        <div className="text-sm font-extrabold uppercase tracking-[0.14em] text-teal">{transition.from}</div>
        <h2 id="module-transition-title" className="mt-2 font-display text-3xl font-extrabold text-navy">Moving to {transition.to}</h2>
        <p className="mt-3 text-lg font-bold leading-relaxed text-navy/75">{transition.summary}</p>
        <button className="btn-primary mt-6 min-h-[60px] w-full text-lg" onClick={dismiss}>Continue to {transition.to}</button>
      </div>
    </div>
  )
}
