...
function Jar({ jarKey, label, color, pos }) {
  ...
  useFrame(() => {
    ...
    if (selector.current) { selector.current.visible = near; selector.current.material.opacity = Math.sin(Date.now() * 0.008) * 0.18 + 0.72 }
    if (body.current) {
  ...
  return (
    <group position={[pos[0], 1.0, pos[1]]}>
      ...
      <mesh ref={selector} position={[0, 0.42, 0]} rotation={[-Math.PI / 2, 0, 0]} visible={false}><ringGeometry args={[0.42, 0.56, 32]} /><meshBasicMaterial color={color} transparent opacity={0.8} depthWrite={false} /></mesh>
