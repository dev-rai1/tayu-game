...
export function MoneyGardenFlowGuide() {
  ...
  const clueSequence = useMemo(() => {
    ...
    const evidence = moneyGardenClues(decisionWeek, mg)
    return [...evidence, guide.instruction]
  ...
  // Money Garden result cards are the single post-choice teaching moment. Make
  // the takeaway unmistakable there instead of adding another competing popup.
  useEffect(() => {
    ...
    <div className="pointer-events-none fixed inset-0 z-[410]">
      {canShowClue && (
        <section
          ...
          className="pointer-events-auto absolute bottom-[calc(10.75rem+env(safe-area-inset-bottom,0px))] left-3 w-[min(90vw,23rem)] rounded-2xl border-2 border-electric/35 bg-white/96 p-4 text-navy shadow-2xl backdrop-blur-sm sm:bottom-4 sm:left-4"
        >
          <div className="flex items-center justify-between gap-2">
            <div className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-electric">
              One clue at a time
            </div>
            <div className="rounded-full bg-navy/8 px-2 py-1 text-[10px] font-extrabold text-navy/65">
              {currentIndex + 1} of {clueSequence.length}
            </div>
          </div>
          ...
          <h2 className="mt-1 font-display text-lg font-extrabold leading-tight">{guide.title}</h2>
          ...
          {[4, 5, 7].includes(Number(decisionWeek)) && !isLastClue && (
            <p className="mt-2 text-xs font-bold leading-snug text-navy/60">
              The storefront stays visible behind this card so you can actually inspect the evidence.
            </p>
          )}
          ...
          <div className="mt-3 flex gap-2">
            {currentIndex > 0 && (
              <button
                type="button"
                onClick={() => setClueIndex((index) => Math.max(0, index - 1))}
                className="min-h-[50px] rounded-xl bg-navy/10 px-4 text-sm font-extrabold text-navy active:scale-95"
              >
                Back
              </button>
            )}
