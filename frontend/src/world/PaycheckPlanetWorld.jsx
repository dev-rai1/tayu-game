...
function TaxCenterBuilding({ active }) {
  return (
    <group>
      ...
      <Billboard position={[0, 6.3, 7.95]}><mesh><planeGeometry args={[8.4, 2.05]} /><meshBasicMaterial map={labelTexture('MODULE 7 · TAYU TAX OFFICE', { bg: '#071748', color: '#ffffff', accent: '#ff9a52' })} transparent toneMapped={false} depthTest={false} /></mesh></Billboard>
...
export function PaycheckPlanetWorld() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener(TAX_WORLD_EVENT, react)
      if (timer) window.clearTimeout(timer)
    }
  }, [])
  ...
  return (
    <group position={[TAX_DISTRICT[0], 0, TAX_DISTRICT[1]]}>
      ...
      <Billboard position={[...local(TAX_POINTS.guide).slice(0, 2), 0]} />
