...
function normalizeStake(value) {
  return Math.max(12, Math.round(Number(value || 0)))
}
...
export function bondOutcome(stake, allocation) {
  ...
  return {
    ...
    finish: r2(principal + interest),
    muniInvested: Number(allocation?.muni || 0) > 0,
  }
}
...
export const useBondStreet = create((set, get) => ({
  ...
  begin: (stake) => {
    ...
    if (saved && saved.completed !== true && Number(saved.stake) === nextStake) {
      set({
        ...
        nearby: false,
        stake: nextStake,
        allocation: { ...blankAllocation(), ...(saved.allocation || {}) },
  ...
  setNearby: (nearby) => set({ nearby: Boolean(nearby) }),

  startAllocation: () => {
    set({ phase: 'allocate', panelOpen: true })
    const s = get()
    saveProfile({ bondStreetProgress: { stake: s.stake, phase: 'allocate', allocation: s.allocation, completed: false } })
  },
  ...
  setAllocation: (id, rawValue) => {
    ...
    const s = get()
    const stake = s.stake
    const value = Math.max(0, Math.min(stake, Math.round(Number(rawValue || 0))))
    const others = BOND_TYPES.map((bond) => bond.id).filter((bondId) => bondId !== id)
    const rest = stake - value
    ...
    set({ allocation })
    saveProfile({ bondStreetProgress: { stake, phase: s.phase, allocation, completed: false } })
  },

  autoSplit: () => {
    const stake = get().stake
    const treasury = Math.round(stake * 0.4)
    const muni = Math.round(stake * 0.3)
    const allocation = { treasury, muni, corporate: stake - treasury - muni }
    set({ allocation })
  },

  confirmAllocation: () => {
    const s = get()
    const total = Object.values(s.allocation).reduce((sum, value) => sum + Number(value || 0), 0)
    ...
    saveProfile({
      bondStreetProgress: { stake: s.stake, phase: 'results', allocation: s.allocation, result, completed: false },
      muniBondInvested: result.muniInvested,
      bondAllocation: s.allocation,
    })
    return true
  },
  ...
  showHarvest: () => {
    const s = get()
    set({ phase: 'harvest', panelOpen: true })
    saveProfile({ bondStreetProgress: { stake: s.stake, phase: 'harvest', allocation: s.allocation, result: s.result, completed: false } })
  },
  ...
  complete: () => {
    ...
    saveProfile({
      badges,
      muniBondInvested: Boolean(s.result?.muniInvested),
      bondAllocation: s.allocation,
      bondStreetResult: s.result,
      bondStreetProgress: { stake: s.stake, phase: 'complete', allocation: s.allocation, result: s.result, completed: true },
    })
    set({ phase: 'complete', panelOpen: false })
  },

  reset: () => {
    set({ phase: 'intro', panelOpen: false, nearby: false, stake: 30, allocation: blankAllocation(), result: null })
    saveProfile({ bondStreetProgress: null, bondStreetResult: null, bondAllocation: null, muniBondInvested: false })
  },
}))
