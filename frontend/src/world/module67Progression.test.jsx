...
describe('Modules 6 and 7 progression', () => {
  ...
  it('lets standalone Module 6 progress even without a Money Garden balance', () => {
    ...
    const runButton = screen.getByRole('button', { name: /Run the bond outcomes/i })
    ...
    fireEvent.click(screen.getByRole('button', { name: /Run the bond outcomes/i }))

    expect(screen.getByRole('heading', { name: /Interest arrives/i })).toBeInTheDocument()
  })

  it('replays Module 6 even when Bond Street was completed before', () => {
    localStorage.setItem('tayu-profile-v1', JSON.stringify({
      badges: ['bond'],
      bondStreet: { completed: true },
      rexTaxIntroSeen: true,
    }))
    sessionStorage.setItem(TAX_ORIGIN_KEY, 'module-select')
    sessionStorage.setItem(BOND_ONLY_KEY, '1')
