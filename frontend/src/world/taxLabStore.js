...
export const useTaxLab = create((set, get) => ({
  ...
  restore: ({ phase = 'case', taxCase = null, stepNumber = 1 } = {}) => {
    ...
    set({
      ...
      feedback: null,
      nearbyAction: null,
      worldNotice: restoredPhase === 'steps' ? `Continue Module 7 inside the Tax Office at ${taxStationForStep(restoredStep).label}.` : 'Meet Ari, Sam, or Jordan inside the Tax Office and choose a taxpayer.',
      work: emptyTaxWork(),
    })
  },
  ...
  chooseCase: (taxCase) => {
    ...
    set({
      phase: 'steps', taxCase, candidateCase: null, stepNumber: 1, panel: null, feedback: null, nearbyAction: null,
  ...
  advanceStep: () => {
    ...
    set((state) => {
      if (state.stepNumber >= 6) {
        return { stepNumber: 6, panel: null, feedback: null, nearbyAction: null, worldNotice: 'Return reviewed. Watch Rex and the E-FILE desk react, then finish the filing.' }
      }
      const next = state.stepNumber + 1
      const nextStation = taxStationForStep(next)
      return { stepNumber: next, panel: null, feedback: null, nearbyAction: null, worldNotice: `Nice work. The office reacted to your decision. Now walk to ${nextStation.label}.` }
    })
  },
